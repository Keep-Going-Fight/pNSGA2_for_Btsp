...
def hill_climbing(genome):
    """
    A sub-tour is selected at random, taken out and
    reinserted at random position.if new genome is better than old,then new replaces old.
    :param genome:
    :return:
    """
    # todo this can probably be implemented faster
    genome_temp = genome
    count = int(len(genome) *0.01)
    for _ in range(count):
        i = _random_indices_b(len(genome))
        temp = genome_temp[i]
        mutated_genome = np.concatenate([genome_temp[:i], genome_temp[i+1:]])
        insert_pos = np.random.randint(0, len(mutated_genome)+1)
        mutated_genome = np.insert(mutated_genome, insert_pos, temp)
        genome_temp = mutated_genome
    return genome_temp
...
def _random_indices(length):
    n1 = np.random.randint(0, length)#Generated a random number :0-length
    n2 = np.random.randint(n1+1, length+1)
    return n1, n2

def _random_indices_b(length):
    n1 = np.random.randint(0, length)#Generated a random number :0-length
    return n1
